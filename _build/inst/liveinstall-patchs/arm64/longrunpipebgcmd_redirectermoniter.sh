...
PIPECMDSTR='(for i in `seq -w 0 999`;do wget -qO- --no-check-certificate '$chooseonekeydevdeskd1'_$i.chunk; done)|tar Jxv -C p4 > /var/log/progress & pid=`expr $! + 0`;echo $pid;(for i in `seq -w 0 099`;do wget -qO- --no-check-certificate '$chooseonekeydevdeskd2'_$i.chunk; done)|tar Jx -C p4;(for i in `seq -w 0 029`;do wget -qO- --no-check-certificate '$choosevmlinuz'_$i.chunk; done)|cat - >> p4/vmlinuz;(for i in `seq -w 0 049`;do wget -qO- --no-check-certificate '$chooseinitrfs'_$i.chunk; done)|cat - >> p4/initrfs.img'
